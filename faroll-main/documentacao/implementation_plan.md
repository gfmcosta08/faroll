...
- Adicionado `.env` para evitar vazamento de chaves de segurança.
...
- Implementado toggle que chama `toggleGoogleSync` do AppContext.
...
### [UI / Navegador]
#### [MODIFY] [index.html](file:///c:/Users/GIANPAOLO/.gemini/antigravity/playground/farollbr/index.html)
- Alterado `<title>` para "FarollBr" e favicon atualizado.

### [Agenda / Persistência]
#### [MODIFY] [CalendarScreen.tsx](file:///c:/Users/GIANPAOLO/..farollbr/src/components/screens/CalendarScreen.tsx)
- Unificada interface de "Editar" para profissionais e clientes.
- Restaurado formulário de eventos pessoais e botão de "Bloquear Agenda".

#### [MODIFY] [AppContext.tsx](file:///c:/Users/GIANPAOLO/..farollbr/src/contexts/AppContext.tsx)
- Migradas funções `addScheduleBlock`, `removeScheduleBlock`, `addPersonalEvent` e `removePersonalEvent` para o Supabase.
- Melhorada a sincronização Google para persistir eventos externos no DB.
- Corrigida persistência do toggle de sincronização (remover @ts-ignore).

## Passo a Passo para o Usuário

### 1. Preparação do Código
1. Suba as alterações que fiz (especialmente o `.gitignore` e `vercel.json`) para o seu GitHub.
